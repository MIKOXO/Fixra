...
const defaultValues = {
  address: { region: '', city: '', woreda: '', kebele: '', houseNumber: '', landmark: '' },
  propertyType: '',
  floors: '',
  floorNumber: '',
  yearBuilt: '',
...
const AddPropertyModal = ({ isOpen, onClose }) => {
  ...
  useEffect(() => {
    ...
    if (propertyType in typeFloorsMap) {
      setValue('floors', typeFloorsMap[propertyType]);
    } else {
      setValue('floors', '');
    }
  ...
  const titleDeedDrop = useDropzone({
    onDrop: (files) => { setTitleDeedFile(files[0]); setTitleDeedError(''); },
  ...
  const floorPlanDrop = useDropzone({
    onDrop: (files) => { setFloorPlanFile(files[0]); setFloorPlanError(''); },
    onDropRejected: (rejections) => {
      const err = rejections[0]?.errors[0];
  ...
  const handleSave = async () => {
    ...
    try {
      let documents = { titleDeed: '', floorPlan: '', photos: [] };
      ...
      if (uploadEntries.length > 0) {
        ...
        documents = {
          titleDeed: result.titleDeedUrl || '',
          floorPlan: result.floorPlanUrl || '',
          photos: result.photosUrls || [],
        };
      }
      ...
      setServerError(err?.message || err || 'Failed to create property');
    } finally {
      setIsSaving(false);
    }
  };
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <NumberInput
                ...
                min={1}
                max={99}
                disabled={propertyType in typeFloorsMap}
            ...
            {propertyType in typeFloorsMap && propertyType && (
        ...
        {step === 2 && (
          <div className="overflow-y-auto max-h-[340px] space-y-3 pr-1 -mr-1">
            <div>
              ...
              {renderDropzone(titleDeedDrop, titleDeedFile, () => { setTitleDeedFile(null); setTitleDeedError(''); }, 'Drop PDF or image here, or click to browse')}
            ...
            <div>
              ...
              {renderDropzone(floorPlanDrop, floorPlanFile, () => { setFloorPlanFile(null); setFloorPlanError(''); }, 'Drop PDF or image here, or click to browse')}
            ...
            <div>
              ...
              <div {...photosDrop.getRootProps()} className={`${dropzoneBase} ${photosDrop.isDragActive ? dropzoneActive : dropzoneIdle}`}>
                ...
                <div className="flex flex-col items-center gap-1 font-body text-sm text-charcoal-400">
                  ...
                  <span className="text-[11px]">{photosFiles.length}/5 selected</span>
                </div>
              </div>
